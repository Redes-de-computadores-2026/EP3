...
struct Config {
    ...
    int seed_base = 42;
    std::string saida = "experimentos/perda.csv";
};
...
static Resultado rodar_cenario(double p_perda, int repeticao, const Config& cfg) {
    ...
    res.quadros_perdidos = c1.instrumentacao().obter_ou_zero("quadros_perdidos")
        ...
        + c3.instrumentacao().obter_ou_zero("quadros_perdidos");
...
static void escrever_csv(const std::string& path, const std::vector<Resultado>& resultados) {
    ...
    out << "p_perda,repeticao,mensagens_planejadas,mensagens_entregues,erro_conexao,timeout,"
           "retransmissoes,dados_enviados,quadros_enviados,quadros_perdidos,pacotes_reencaminhados,"
    ...
    for (const auto& r : resultados) {
        ...
        double taxa_retrans = r.mensagens_planejadas > 0
            ...
            : 0.0;
        double perda_quadros = r.quadros_enviados > 0
            ? static_cast<double>(r.quadros_perdidos) / r.quadros_enviados
            : 0.0;
        out << r.p_perda << ","
            << r.repeticao << ","
